...
<body>
<h1>制度管理</h1>
<a href="<?php echo U('Departadmin/newinstitution');?>">新建社团制度</a>
<hr>
<table border="2px solid" id="message">
    <?php if(is_array($res)): foreach($res as $k=>$value): ?><tr>
            ...
            <td data-id="<?php echo ($value["institutionid"]); ?>">
                <a href="<?php echo U('Departadmin/institutionmodify',array('id'=>$value['institutionid']));?>">
                    <button>修改</button></a>
                |
...
<script>
    $('.institutiondel').on('click',function () {
        ...
        var url = "<?php echo U('Departadmin/institutiondel');?>";
        $.ajax({
            url: url,
            data: param,
            dataType: 'json',
